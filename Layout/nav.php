
<nav class="d-flex justify-content-between py-3 px-5 border border-bottom-2">
        <div class="d-flex align-item-center">
            <img src="./images/Logo.png" class="rounded me-3" style="width: 50px;height: 50px;">
            <h3><a href="" style="text-decoration:none;" class="text-dark">Tee World Myanmar</a></h3>
        </div>
        <div>
            <ul class="nav me-4">
                <li class="nav-item">
                    <a href="" class="nav-link text-dark">Home</a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link text-dark">T-shirts</a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link text-dark">Store</a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link text-dark">Cart</a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link text-dark">History</a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link text-dark">My account</a>
                </li>
            </ul>
        </div>
    </nav>
